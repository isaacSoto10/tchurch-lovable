...
- `ASC_TARGET_MARKETING_VERSION`: Optional App Store version filter. The current workflow pins this to `4.0.16` so another release train cannot be promoted by accident.
- `ASC_TARGET_BUILD_NUMBER`: Optional build-number filter. The current workflow pins this to `226` so later CI-only builds cannot replace the tested build.

For the `4.0.16` release, Xcode Cloud must run with `CI_BUILD_NUMBER=226`.
...
ASC_TARGET_MARKETING_VERSION=4.0.16 \
ASC_TARGET_BUILD_NUMBER=226 \
ASC_DRY_RUN=true \
