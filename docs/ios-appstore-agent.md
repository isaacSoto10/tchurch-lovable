...
- `ASC_TARGET_MARKETING_VERSION`: Optional App Store version filter. The current workflow pins this to `4.0.13` so another release train cannot be promoted by accident.
- `ASC_TARGET_BUILD_NUMBER`: Optional build-number filter. The current workflow pins this to `212` so later CI-only builds cannot replace the tested build.

For the `4.0.13` release, Xcode Cloud must run with `CI_BUILD_NUMBER=212`.
...
ASC_TARGET_MARKETING_VERSION=4.0.13 \
ASC_TARGET_BUILD_NUMBER=212 \
ASC_DRY_RUN=true \
