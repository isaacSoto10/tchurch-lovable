...
export function normalizeStudioLANUpdate(value: unknown): StudioLANUpdate | null {
  ...
  if ((channel !== "audience" && channel !== "stage") || !sequence || !UINT64.test(sequence) || !revision || !UINT64.test(revision)
    ...
    || !Number.isSafeInteger(cueCount) || cueCount < 0 || audienceCue === undefined) return null;
  ...
  if (source.stage != null) {
    ...
    if (!sourceStage || nextCue === undefined || (sourceStage.message != null && !message)
    ...
    stage = { nextCue, chordLines: chordLines as string[], timers: timers as StudioLANTimer[], message };
  }
