...
import { Plus, Pencil, Trash2, Search, ChevronUp, ChevronDown, Music, FileText, Bell, X, Check, Clock, Users, ExternalLink, PlayCircle, GripVertical, FileDown, Maximize2 } from "lucide-react";
...
export default function Services() {
  ...
  const chartItem = chartSelection
    ...
    : null;
  const chartMusicAssignmentDetails = chartSelection
    ? getMusicAssignmentSummaries(serviceAssignments[chartSelection.serviceId] || [], 8)
    : [];
          ...
          {chartItem?.song && (
            <>
              <DialogHeader className="border-b border-zinc-100 px-2.5 py-1.5 text-left sm:px-3 sm:py-2">
                ...
                {chartMusicAssignmentDetails.length > 0 && (
                  <div className="flex gap-1 overflow-x-auto pb-0.5 pt-1">
                    {chartMusicAssignmentDetails.map((assignment) => (
                      ...
                      {itemsLoading[svc.id] ? (
                        ...
                        <div className="space-y-2">
                          {(serviceItems[svc.id] || []).map((item, idx) => {
                            ...
                            const serviceMusicAssignments = getMusicAssignments(serviceAssignments[svc.id] || []);
                            const assignmentDetails = getMusicAssignmentSummaries(serviceAssignments[svc.id] || [], 8);
                            return (
                              <div
                                key={item.id}
                                    ...
                                    <div className="min-w-0 flex-1">
                                      <div className="flex items-start gap-2">
                                        ...
                                        <div className="flex shrink-0 items-center justify-end gap-1">
                                          {displayKey && (
                                            <Badge variant="secondary" className="hidden rounded-full px-2 py-1 text-[11px] sm:inline-flex sm:text-xs">Tono {displayKey}</Badge>
                                          )}
                                          {isSong && (
                                            <Button
                                              variant="ghost"
                                      ...
                                      <div className="mt-2 flex flex-wrap items-center gap-1.5 sm:gap-2">
                                        {displayKey && (
                                          <Badge variant="secondary" className="rounded-full px-2 py-0.5 text-[11px] sm:hidden">Tono {displayKey}</Badge>
                                        )}
                                        ...
                                        {isPlanner && (
                                          <Button
                                            variant="ghost"
                                            size="icon"
                                            className={`${isSong && !expandedSongItems[item.id] ? "hidden sm:inline-flex" : "inline-flex"} ml-auto h-8 w-8 rounded-xl text-zinc-400 hover:text-red-500 sm:h-9 sm:w-9`}
                                            onClick={(event) => {
                                              event.stopPropagation();
                                      ...
                                      {isSong && expandedSongItems[item.id] && assignmentDetails.length > 0 && (
                                        <div className="mt-2 rounded-xl bg-primary/5 px-2.5 py-2 ring-1 ring-primary/10">
                                          ...
                                          <div className="mt-1 flex flex-wrap gap-1.5">
                                            {assignmentDetails.slice(0, 5).map((assignment) => (
                                      ...
                                      <div className="flex items-center justify-between gap-3">
                                        <div className="min-w-0">
                                          ...
                                          <p className="mt-0.5 text-xs leading-5 text-zinc-500">
                                            Ábrela en pantalla amplia para ver acordes y letra sin encimarse.
                                          </p>
                                        </div>
                                        <Button
                                          type="button"
